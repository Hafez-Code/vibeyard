...
- **Current providers**: `ClaudeProvider` (`src/main/providers/claude-provider.ts`) — extracts all Claude-specific logic from `pty-manager.ts`, `prerequisites.ts`, `claude-cli.ts`, and `hook-status.ts`. Also `CodexProvider` (`codex-provider.ts`), `CopilotProvider` (`copilot-provider.ts`, agents use `.agent.md`), `GeminiProvider` (`gemini-provider.ts`), and `OpencodeProvider` (`opencode-provider.ts` — wraps the OpenCode TUI; resume via `--session <id>`, initial prompt via `--prompt`, profile isolation via `OPENCODE_CONFIG_DIR`, config read from `~/.config/opencode/opencode.json` + `.opencode/` with agents in `~/.config/opencode/agents/`; `hookStatus`/`costTracking` are false and on-disk transcript indexing is not wired because OpenCode stores sessions in a SQLite DB rather than JSONL/JSON).
